function multiUploader(config){
	...
	this.all = []
	var self = this;
    var basurl=this.config.imgpath; 
		...
		if(this.items.length > 0){
			...
			var uId = "";
 			for(var i = 0; i<this.items.length; i++){
				...
				var sampleIcon = '<img src="'+basurl+'images/image.png" />';
				var errorClass = "";
				if(typeof this.items[i] != undefined){
					if(self._validate(this.items[i].type) <= 0) {
						sampleIcon = '<img src="'+basurl+'images/unknown.png" />';
						errorClass =" invalid";
					} 
					html += '<div class="dfiles'+errorClass+'" rel="'+uId+'"><h5>'+sampleIcon+this.items[i].name+'</h5><div id="'+uId+'" class="progress" style="display:none;"><img src="'+basurl+'images/ajax-loader.gif" /></div></div>';
				}
			}
