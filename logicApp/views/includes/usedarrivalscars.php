<section>
  <p>
    <div class="holder3"></div>
      <hr>
        <div class="clear"></div>
            <ul id="itemContainer3">
                <?PHP  $onroad_usedCar = $this->db->query("SELECT `onroad_vehicle_details`.`pro_id`,`exp_prize`,`onroad_vehicle_images`.`image`,`title_name`,`companyname`,`model_name`,`kilo_meter` 
                ...
                left outer join `onroad_vehicle_images` on `onroad_vehicle_images`.`pro_id`=`onroad_vehicle_details`.`pro_id` 
                where `onroad_vehicle_details`.`vtype_id`='2' and `onroad_vehicle_details`.`status`!='0' group by `onroad_vehicle_images`.`pro_id` order by `onroad_vehicle_details`.`pro_id` DESC LIMIT 21 ");
...
if ($onroad_usedCar->num_rows() > 0)
{
    ...
    { ?>
        ...
        {
            echo IMAGES_PATH . '/page1_img1.jpg';
        } ?>') ;">
 <div class="col-xs-15" style="padding:90px; padding-left:66px;">
<a href="#myModal" class="btn details fullview" get="view" value="<?=$usCar->pro_id?>"  data-toggle="modal" data-target="#myModal" id="<?=$usCar->pro_id?>" >details</a></div></div>
                            ...
                            <div class="price"> &#x20B9;<?PHP echo $usCar->
exp_prize; ?><span></span></div>
